...
for message in consumer:
    ...
    if len(hex_data) > 0:
        ...
        print(hex_data2)
        print("str",hex_data2[9:11])

        if hex_data2[9:11] == '12':
        ...
        try:
            ...
            ltocur.close()
            continue
        sql = "INSERT INTO ioeSt2BatteryData(batteryVoltage,batteryCurrent,mainContactorStatus,prechargeContactorStatus,batteryStatus,packSOC,packUsableSOC,chargingEnergy,dischargingEnergy,availableEnergy,recordTimestamp) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        print(val)
        try:
            ltocur.execute(sql,val)
            unprocesseddb.commit()
                # print(val)
            print("IOE String3 Battery data inserted")
            ltocur.close()
            unprocesseddb.close()
        except Exception as ex:
